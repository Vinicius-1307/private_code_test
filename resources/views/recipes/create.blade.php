        ...
        <form method="POST" action="{{ route('recipes.store') }}">
            ...
            <div class="mb-4">
                ...
                <input type="text" class="form-control @error('title') is-invalid @enderror" id="title"
                    name="title" value="{{ old('title') }}" placeholder="Ex: Bolo de Chocolate" required maxlength="120">
                @error('title')
            ...
            <div class="mb-4">
                ...
                <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description"
                    rows="3" placeholder="Breve descrição sobre a receita...">{{ old('description') }}</textarea>
                @error('description')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
                    ...
                    @else
                        <div class="input-group mb-2 ingredient-item">
                            ...
                            <input type="text" class="form-control" name="ingredients[]"
                                placeholder="Ex: 2 xícaras de farinha de trigo" required>
                            <button class="btn btn-outline-danger remove-ingredient" type="button" disabled>
                                <i class="bi bi-trash"></i>
                            </button>
                        </div>
                    @endif
            ...
            <div class="mb-4">
                ...
                <textarea class="form-control @error('steps') is-invalid @enderror" id="steps" name="steps" rows="8"
                    placeholder="Descreva o passo a passo para preparar a receita..." required>{{ old('steps') }}</textarea>
                @error('steps')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
